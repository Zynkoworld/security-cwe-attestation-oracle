#!/usr/bin/env python3
"""security-cwe-attestation-oracle -- a deterministic attestation-integrity decider for the `security-cwe` knowledge domain.

An **oracle** *deterministically decides* the truth of a case. This one decides, for a `security-cwe` knowledge
capsule, whether it is a well-formed, provenance-carrying attestation. VALID iff: domain=='security-cwe', a non-empty
...
def decide(c):
    if c.get("domain") != "security-cwe": return "INVALID"
...
def _run_probes(path):
    ...
    print("security-cwe attestation oracle: probes=%d | recall=%.3f | false_positives=%d | verdict=%s"%(len(probes),recall,fp,"PASS" if ok else "FAIL"))
